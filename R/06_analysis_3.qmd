...
```{r}
tt_tumor_vs_Healthy <- topTable(
  fit_all,
  coef   = "diagnosisHealthy",
  number = Inf,
  sort.by = "P"
)
head(tt_tumor_vs_Healthy)
```

**View Differential Expression Results**

```{r}
tt_tumor_vs_Healthy <- tt_tumor_vs_Healthy |>
...
```{r}
signif_genes_count <- tt_tumor_vs_Healthy |> 
...
```{r}
DE_results <- list(
  Tumor_vs_Healthy = tt_tumor_vs_Healthy
)
```
...
universe_symbols <- tt_gene |> pull(symbol)
...
```{r}
genes <- reactome_df|> pull(gene_symbol) |> unique()  |>  sort()
paths <- reactome_df |>  pull(gs_name) |> unique() |>  sort()

gi <- reactome_df |> 
  mutate(gene_factor = factor(gene_symbol, levels = genes))  |> 
  pull(gene_factor)  |> 
  as.integer()

pi <- reactome_df |> 
  mutate(path_factor = factor(gs_name, levels = paths))  |> 
  pull(path_factor)  |> 
  as.integer()
```
...
From the annotated network, we extract the top 20 upregulated and top 20 downregulated genes. These represent highly differentiated genes in the Tumor vs Healthy contrast and are used for summarizing key biological processes in the data.
