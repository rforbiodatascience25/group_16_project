...
hub_logical <- get_hub_logical(g, q = 0.90)  # Hubs = top 10% by degree
...
key_modules %>% dplyr::select(module, module_size, n_sig_pathways, top_q)
  ...
  # Adding a label for every gene :is it inside a key module?
nodes_out <- nodes_out %>%
  mutate(
    in_key_module = module %in% key_modules$module
  ) %>%
  relocate(in_key_module, .after = module)  # relocating in specific position
...
# Top 20 most upregulated DE genes (FDR < 0.05)
top20_up <- nodes_out %>%
...
# Top 20 most downregulated DE genes (FDR < 0.05)
...
# Final top 40 genes
combined_40 <- bind_rows(top20_up, top20_down) %>%
  arrange(desc(abs(logFC))) #ordered by strongest change first

View(combined_40)

# 2) Saving ranked top 40 in csv format
write_csv(
  combined_40,
  file.path(output_dir, paste0(tag, "_top40_DE.csv"))
)
```